...
static int goodix_tptest_prepare(struct goodix_ts_test *ts_test)
{
	...
	if (cfg->len > 0) {
		...
		if (ret < 0) {
			ts_err("Send test config failed 1, again Send test config");

			goodix_ts_power_off(ts_test->ts);
			usleep_range(5000, 5100);
			goodix_ts_power_on(ts_test->ts);
			ret = ts_test->ts->hw_ops->reset(ts_test->ts, GOODIX_NORMAL_RESET_DELAY_MS);
			if (ret) {
				ts_err("Failed to request reset gpio, ret:%d", ret);
			}

			ret = ts_test->ts->hw_ops->send_config(ts_test->ts, cfg->data, cfg->len);
			if (ret < 0) {
				ts_err("Send test config failed 2, exit");
