...
static int tpd_set_smart_cover(struct ztp_device *cdev, int enable)
{
	...
	FTS_INFO("smart_cover mode is %d", enable);
	if (enable) {
		ret = fts_write_reg(FTS_REG_COVER_MODE_EN, 1);
		FTS_INFO("enter_smart_cover success\n");
	} else {
		ret = fts_write_reg(FTS_REG_COVER_MODE_EN, 0);
		FTS_INFO("leave_smart_cover success!\n");
	}
