...
int cam_ois_dw9781c_fw_download_init(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	total_words = INIT_WRITE_SIZE;  //word
	...
	for (cnt = 0; cnt < i2c_reg_setting.size; cnt++) {
		i2c_reg_setting.reg_setting[cnt].reg_addr = g_ois_dw9781c_init[cnt].reg_addr;
		i2c_reg_setting.reg_setting[cnt].reg_data = g_ois_dw9781c_init[cnt].reg_data;
		i2c_reg_setting.reg_setting[cnt].delay = g_ois_dw9781c_init[cnt].delay;
		i2c_reg_setting.reg_setting[cnt].data_mask = g_ois_dw9781c_init[cnt].data_mask;
		...
		if (rc < 0)
		{
			CAM_ERR(CAM_OIS, "dw9781c i2c write failed %d", rc);
		}
	}
...
int cam_ois_dw9784_fw_download_init(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	/*init for download firmware*/
	total_words = DW9784_INIT_WRITE_SIZE;  //word
	i2c_reg_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
	i2c_reg_setting.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
	...
	for (cnt = 0; cnt < i2c_reg_setting.size; cnt++) {
		i2c_reg_setting.reg_setting[cnt].reg_addr = g_ois_dw9784_init[cnt].reg_addr;
		i2c_reg_setting.reg_setting[cnt].reg_data = g_ois_dw9784_init[cnt].reg_data;
		i2c_reg_setting.reg_setting[cnt].delay = g_ois_dw9784_init[cnt].delay;
		i2c_reg_setting.reg_setting[cnt].data_mask = g_ois_dw9784_init[cnt].data_mask;
...
int cam_ois_dw9781c_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(CAM_OIS, "cam_ois_dw9781c_fw_download %s Entry", o_ctrl->ois_name);
	...
	if(!cam_ois_dw9781c_need_update_fw(o_ctrl))
	{
	    CAM_INFO(CAM_OIS, "OIS dw9781c firmware is latest version[0x104, 0x116] no need to update!");
		return rc;
	}
	...
	cam_ois_dw9781c_fw_download_init(o_ctrl);
...
int cam_ois_dw9784_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(CAM_OIS, "cam_ois_dw9784_fw_download %s Entry", o_ctrl->ois_name);
	...
	if((strcmp(o_ctrl->ois_name, "ois_dw9784") == 0) && (!cam_ois_dw9784_need_update_fw(o_ctrl)))
	{
	    CAM_INFO(CAM_OIS, "OIS dw9784 firmware is latest version[0x603, 0x628] no need to update!");
		return rc;
	}

	if((strcmp(o_ctrl->ois_name, "ois_dw9784_hi847") == 0) && (!cam_ois_dw9784_hi847_need_update_fw(o_ctrl)))
	{
	    CAM_INFO(CAM_OIS, "OIS dw9784_hi847 firmware is latest version[0x202, 0x629] no need to update!");
		return rc;
	}
	...
	cam_ois_dw9784_fw_download_init(o_ctrl);
	...
	CAM_INFO(CAM_OIS, "cam_ois_dw9784_fw_download Exit");
...
uint32_t cam_ois_dw9781c_firmware_download(struct cam_ois_ctrl_t *o_ctrl)
{
    ...
    // init
	for (cnt = 0; cnt < INIT_WRITE_SIZE; cnt++) {
		rc = zte_cam_cci_i2c_write(&(o_ctrl->io_master_info),
			g_ois_dw9781c_init[cnt].reg_addr, 
			g_ois_dw9781c_init[cnt].reg_data,
			addr_type,
			data_type);
		msleep(g_ois_dw9781c_init[cnt].delay);
...
uint32_t cam_ois_dw9784_firmware_download(struct cam_ois_ctrl_t *o_ctrl)
{
    ...
    // init
	for (cnt = 0; cnt < DW9784_INIT_WRITE_SIZE; cnt++) {
		rc = zte_cam_cci_i2c_write(&(o_ctrl->io_master_info),
			g_ois_dw9784_init[cnt].reg_addr, 
			g_ois_dw9784_init[cnt].reg_data,
			addr_type,
			data_type);
		msleep(g_ois_dw9784_init[cnt].delay);

		if (rc < 0)
		{
			CAM_ERR(CAM_OIS, "dw9784 i2c write failed %d", rc);
		}
	}
...
uint32_t cam_ois_dw978x_firmware_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (strcmp(o_ctrl->ois_name, "ois_dw9781c") == 0)
		data = cam_ois_dw9781c_firmware_download(o_ctrl);
	else if ((strcmp(o_ctrl->ois_name, "ois_dw9784") == 0) || (strcmp(o_ctrl->ois_name, "ois_dw9784_hi847") == 0))
		data = cam_ois_dw9784_firmware_download(o_ctrl);
	else
		CAM_ERR(CAM_OIS, "%s unknown ois name Exit\n", __func__);
