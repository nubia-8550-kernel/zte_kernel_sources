...
#define MARGIN_LEVELS 2
unsigned int sched_early_up_migrate[MARGIN_LEVELS];
unsigned int sched_early_down_migrate[MARGIN_LEVELS];
  ...
  if(wts){
      ...
      if(wts->scp_s == NEED_RESTORE)//恢复sched_class
      {
        if(rt_policy(p->policy)){
		  ...
		  if(rt_policy(p->policy)){
            printk(KERN_INFO "[scp_work]%d thread %s --restore cfs sched_class failed!\n", p->pid, p->comm);
          }
        }
...
static void nbia_sched_setaffinity_early(void *unused, struct task_struct *p, const struct cpumask *new_mask, int *retval)
{
    struct walt_task_struct *wts = (struct walt_task_struct *) p->android_vendor_data1;
    if(!wts->is_app)return;
    if(wts->mask_persisted){
...
unsigned int get_early_up_migrate(int index){
   if(index >= MARGIN_LEVELS)return 0;
   //if(scp_udata.debug) {
...
ssize_t store_updown_migrate_handle(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count, bool is_u){
    ...
    if(strstr(chBuffer, pchDilem)){
       pchTmp = chBuffer;
       while((optStr = strsep(&pchTmp, pchDilem)) != NULL && index < 2){
          if(strnlen(optStr, 8)){
       ...
       pchTmp = NULL;
       optStr = NULL;
       if(migrate_value[0] < 0 || migrate_value[1] < 0){
    	 return -EINVAL;
       }  
       mutex_lock(&ud_value_data_lock);
       if(is_u){
         sched_early_up_migrate[0] = migrate_value[0];
         sched_early_up_migrate[1] = migrate_value[1];
         printk(KERN_INFO "store_updown_migrate_handle update up-migrate-value:[%d,%d] is_u:%d \n", sched_early_up_migrate[0], sched_early_up_migrate[1],is_u);
       }else{
         sched_early_down_migrate[0] = migrate_value[0];
         sched_early_down_migrate[1] = migrate_value[1];
         printk(KERN_INFO "store_updown_migrate_handle update down-migrate-value:[%d,%d] is_u:%d \n", sched_early_down_migrate[0], sched_early_down_migrate[1],is_u);
       }    
...
ssize_t show_updown_migrate_handle(struct kobject *kobj, struct kobj_attribute *attr, char *buf, bool is_u){
   ...
   snprintf(event_string, sizeof(event_string), "%d:%d"
     ...
     , sched_nbia_render_tid_array[1]);
   retval = snprintf(buf, sizeof(event_string),"%s", event_string);
     snprintf(event_string, sizeof(event_string), "sched_early_up_migrate:[%d, %d]"
       , sched_early_up_migrate[0]
       , sched_early_up_migrate[1]);
   }else{
     snprintf(event_string, sizeof(event_string), "sched_early_down_migrate:[%d, %d]"
       , sched_early_down_migrate[0]
       , sched_early_down_migrate[1]);
   }    
   retval = snprintf(buf, sizeof(event_string),"%s", event_string);
   mutex_unlock(&ud_value_data_lock);
   return retval;
...
void nbia_init(){
    ...
    memset(sched_early_up_migrate, 0, MARGIN_LEVELS);
    memset(sched_early_down_migrate, 0, MARGIN_LEVELS);
    memset(sched_prio_adjusted_info, 0, PRIO_ADJUSTED_TID_ARRAY_SIZE * 5);
