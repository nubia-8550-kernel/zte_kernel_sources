...
#ifdef CONFIG_TOUCHSCREEN_USE_BOARDID
extern int request_board_id(void);
#define TP_FINAL_BOARD_ID   9
#endif
...
int power_load_board_id(void)
{
	...
	if(board_id <= 0) {
		board_id = TP_FINAL_BOARD_ID;
		FTS_ERROR("request_board_id fail,make board_id to %d\n", board_id);
	}

	FTS_INFO("%s:board_id %d\n", __func__, board_id);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (ret < 0) {
		FTS_ERROR("invalid vdd-enable-gpio in dt: %d", ret);
		return -EINVAL;
	}
    pdata->avdd_enable_gpio = ret;
	FTS_INFO("get vdd-enable-gpio[%d] from dt", ret);
	...
	if (strlen(pdata->iovdd_name)) {
		...
		ret = regulator_set_voltage(pdata->avdd, 1800000, 1800000);
		if (ret < 0) {
			FTS_ERROR("set iovdd voltage failed");
			return ret;
		}
...
#ifdef CONFIG_TOUCHSCREEN_FTS_SPI
...
#ifdef FOR_ZTE_CELL
    fts_write_reg(FTS_REG_REPORT_RATE, 0x0C);
    FTS_INFO("proc enter_120_HZ success\n");
#endif
...
#elif defined(CONFIG_TOUCHSCREEN_FTS_I2C)
