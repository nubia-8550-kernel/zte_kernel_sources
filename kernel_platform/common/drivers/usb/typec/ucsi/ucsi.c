...
static void ucsi_handle_connector_change(struct work_struct *work)
{
	...
	{
		dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change00 post_ack_status.change=0x%04x\n" ,post_ack_status.change);
		goto out_unlock;
	}
	...
	if (con->status.change & UCSI_CONSTAT_POWER_OPMODE_CHANGE ||
		...
		dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change01 con->status.change=0x%04x\n", con->status.change);	
		ucsi_pwr_opmode_change(con);
		ucsi_port_psy_changed(con);
	}

	if (con->status.change & UCSI_CONSTAT_POWER_DIR_CHANGE) {
		dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change02 con->status.change=0x%04x,role=%d\n", con->status.change,role);
	...
	if (con->status.change & UCSI_CONSTAT_CONNECT_CHANGE) {
		dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change03 con->status.change=0x%04x,role=%d\n", con->status.change,role);
		typec_set_pwr_role(con->port, role);
		...
		{
			dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change04 con->status.flagse=0x%04x,u_role=%d\n", con->status.flags,u_role);
			ucsi_register_partner(con);
		}
		else
		{
			dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change05 con->status.flagse=0x%04x,u_role=%d\n", con->status.flags,u_role);
		...
		dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change06 con->status.change=0x%04x,u_role=%d\n", con->status.change,u_role);
		ret = usb_role_switch_set_role(con->usb_role_sw, u_role);
		if (ret)
	...
	{
		dev_err(ucsi->dev,"slf1: ucsi_handle_connector_change07 con->status.change=0x%04x,PARTNER_TYPE=%d\n", con->status.change,UCSI_CONSTAT_PARTNER_TYPE(con->status.flags));
...
void ucsi_connector_change(struct ucsi *ucsi, u8 num)
{
	...
	{
		dev_err(ucsi->dev,"slf1: ucsi_connector_change enter,num=%d\n" ,num);
		schedule_work(&con->work);
	}
}
